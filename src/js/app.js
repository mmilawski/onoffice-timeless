...
jQuery(document).ready(function() {
  ...
  function setWpmlLanguageSwitcherStateModifier() {
    ...
    const $languageToggle = $languageSwitcher.find('.js-wpml-ls-item-toggle.wpml-ls-item-toggle');
    const $subMenu = $languageSwitcher.find('.js-wpml-ls-sub-menu.wpml-ls-sub-menu');
  
    if ($languageToggle.length === 0 || $subMenu.length === 0) {
      console.warn('WPML state modifier aborted: missing element(s):', {
        languageToggle: $languageToggle.length > 0,
    ...
    const addModifier = () => {
      $languageToggle.addClass('--is-open');
    };
  
    const removeModifier = () => {
      $languageToggle.removeClass('--is-open');
    };
  
    let captureHandler = null;
    function attachCaptureHandlerOnce() {
      if (captureHandler) return;
      captureHandler = function docClickCapture(e) {
        if ($(e.target).closest('.c-module-language-switcher').length) {
          return;
        }
    ...
    $languageSwitcher.on('click', function onSwitcherClick(e) {
      if (isMenuVisible()) {
        addModifier();
  
        setTimeout(() => {
          if (isMenuVisible()) attachCaptureHandlerOnce();
        }, 0);
      } else {
        removeModifier();
      }
  ...
  setWpmlLanguageSwitcherStateModifier();
