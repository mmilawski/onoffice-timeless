...
if (!function_exists('renderGdprHintText')) {
    function renderGdprHintText($pForm): string
    {
        ...
        if (
            trim((string) $hintLabel) === '' ||
            $hintLabel === esc_html($defaultLabel) ||
            $hintLabel === $defaultLabel
        ) {
            return '';
        }
        ...
        $hintLabel = preg_replace(
            '/<a\b([^>]*?)\shref=/i',
            '<a$1 target="_blank" rel="noopener noreferrer" href=',
            $hintLabel,
        );
        return '<div class="oo-gdpr-hint-text">' .
            wp_kses_post($hintLabel) .
            '</div>';
    }
}
