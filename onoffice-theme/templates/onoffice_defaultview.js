    ...
    $(document).ready(function () {
      ...
      function initializeDonutChart() {
        ...
        data.forEach((item) => {
          ...
          gradientString += `${item.color} ${start.toFixed(2)}% ${end.toFixed(
            2
          )}%, `;
          start = end;
        });
      ...
      function applyGradientToSegments() {
        const segments = $(
          '.oo-details-energy-certificate .energy-certificate-container .segment'
        );
        ...
        segments.each(function (index) {
          ...
          const normalizedPosition = isInInitialSegment
            ? positionRatio * 2
            : (positionRatio - 0.5) * 2;
          const color = calculateColor(
            isInInitialSegment ? colors.start : colors.middle,
            isInInitialSegment ? colors.middle : colors.end,
            normalizedPosition
          );
          const nextColor = calculateColor(
            isInInitialSegment ? colors.start : colors.middle,
            isInInitialSegment ? colors.middle : colors.end,
            normalizedPosition + 1 / (segments.length - 1)
          );

          $(this).css(
            'background',
            `linear-gradient(to right, rgb(${color.join(
              ','
            )}), rgb(${nextColor.join(',')}))`
          );
        });
      ...
      function calculateColor(start, end, positionRatio) {
        return start.map((startValue, i) =>
          Math.round(startValue + positionRatio * (end[i] - startValue))
        );
      }
    });
  });
